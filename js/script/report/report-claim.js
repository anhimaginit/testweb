...
var reportListInput = {
    availableFields: [],
    customColumns: ["status", "create_by_name", "contact_name", "start_date"],
    listField: {
        status: {
            name: "Status",
            type: "select",
...
var getReportListType = function () {
    $.get('php/getSession.php?data=int_acl&child=acl_rules-ControlListForm-claimreport', function (res) {
        if (res && res != '') {
...
$("#btnDownloadCsvReport").click(function () {
    ...
    _mydata.personal_filter = window.report_type
    $.ajax({
        url: link._reportDownloadCsvClaim,
        type: 'POST',
        data: _mydata,
        success: function (res) {
            var report = JSON.parse(res);
...
$("#btnSaveReport").click(function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            messageForm("The report is saved successfully", true);
            writeLoadOldReport();
        }
    })
})
